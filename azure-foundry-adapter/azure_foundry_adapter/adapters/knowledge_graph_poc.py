...
Written to be copied verbatim into
`knowledge-graph-poc/src/kg/llm/adapters/azure_foundry.py` — it imports that
project's own `kg.config` / `kg.llm.adapters.base` / `kg.llm.errors` modules,
so it will NOT import successfully from inside this `azure-foundry-adapter`
folder. It is kept here (rather than inside the other repo) only because you
asked for the wiring to live in its own folder rather than edited in place —
see `INTEGRATION.md` next to this file for the exact few lines to add to
`kg/config.py` and `kg.yaml` once you do copy it over.
...
class AzureFoundrySettings:
    """Parsed from a new `llm.azure_foundry` block in `kg.yaml` (see INTEGRATION.md).

    `endpoint` is the resource base URL, e.g. `https://<resource>.openai.azure.com`
    (no path, no query string). `models` maps stage name -> Azure deployment name,
