"""Embeddings: local BGE-M3, with an offline fake for tests and dry runs.
...
def get_embedder(model_id: str | None = None) -> Embedder:
    ...
    Honours `POC_FAKE_EMBEDDINGS`; otherwise builds a `BGEM3Embedder` (still
    without loading weights — that happens on first embed call).
    """
